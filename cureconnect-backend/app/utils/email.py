...
def store_otp(email: str, otp: str):
    ...
    try:
        # Write to a file in the backend workspace for easy access
        with open("otp_log.txt", "w") as f:
            f.write(f"EMAIL: {email.lower()}\nOTP_CODE: {otp}\nTIMESTAMP: {datetime.now().isoformat()}\n")
    except Exception as e:
        logger.error(f"Failed to write OTP log file: {e}")
...
def send_otp_email(to_email: str, otp: str, user_name: str = "User") -> bool:
    try:
        ...
        # Development fallback: print the OTP to the console so the developer can copy it
        print("\n" + "="*80)
        print("[DEV FALLBACK] SMTP BLOCKED BY NETWORK/FIREWALL!")
        print(f"TO: {to_email}")
        print(f"OTP CODE: {otp}")
        print("="*80 + "\n")
        logger.info(f"[DEV FALLBACK] Printed OTP {otp} for {to_email} to console.")
        return True